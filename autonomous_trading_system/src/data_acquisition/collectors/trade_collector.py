...
class TradeCollector:
    ...
    def collect_trades(
        ...
        """
        logger.info(f"Collecting trades for {len(tickers)} tickers on {date_to_collect}")
    ...
    def collect_large_trades(
        ...
        """
        logger.info(f"Collecting large trades (min size: {min_size}) for {len(tickers)} tickers on {date_to_collect}")
        ...
        for ticker, df in all_trades.items():
            if not df.empty:
                ...
                if not large_df.empty:
                    ...
                    logger.debug(f"Found {len(large_df)} large trades for {ticker} on {date_to_collect}")
                else:
                    logger.debug(f"No large trades found for {ticker} on {date_to_collect}")
        
        logger.info(f"Completed large trade data collection for {len(large_trades)} tickers")
        return large_trades
    ...
    def collect_block_trades(
        ...
        """
        logger.info(f"Collecting block trades (min value: ${min_value:,.2f}) for {len(tickers)} tickers on {date_to_collect}")
            ...
            if not df.empty:
                ...
                if 'price' in df.columns and 'size' in df.columns:
                    ...
                    if not block_df.empty:
                        ...
                        logger.debug(f"Found {len(block_df)} block trades for {ticker} on {date_to_collect}")
                    else:
                        logger.debug(f"No block trades found for {ticker} on {date_to_collect}")
                else:
                    logger.warning(f"Missing price or size columns for {ticker}")
        
        logger.info(f"Completed block trade data collection for {len(block_trades)} tickers")
        return block_trades
    ...
    def collect_unusual_trades(
        ...
        """
        logger.info(f"Collecting unusual trades (z-score > {z_score_threshold}) for {len(tickers)} tickers on {date_to_collect}")
        ...
        for ticker, df in all_trades.items():
            if not df.empty and len(df) > 10:  # Need enough data to calculate meaningful z-scores
                ...
                if std_size > 0:  # Avoid division by zero
                    ...
                    # Filter trades by z-score
                    unusual_df = df[df['size_z_score'] >= z_score_threshold].copy()
                    
                    if not unusual_df.empty:
                        unusual_trades[ticker] = unusual_df
                        logger.debug(f"Found {len(unusual_df)} unusual trades for {ticker} on {date_to_collect}")
                    else:
                        logger.debug(f"No unusual trades found for {ticker} on {date_to_collect}")
                else:
                    logger.debug(f"Standard deviation of trade sizes is zero for {ticker}")
        
        logger.info(f"Completed unusual trade data collection for {len(unusual_trades)} tickers")
