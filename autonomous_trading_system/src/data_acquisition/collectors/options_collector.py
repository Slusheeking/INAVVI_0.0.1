...
class OptionsCollector:
    ...
    def collect_options_chain(
        ...
        logger.info(f"Completed options chain collection for {len(results)} underlyings")
        return results
    ...
    def collect_options_expirations(
        ...
        """
        logger.info(f"Collecting options expirations for {len(underlyings)} underlyings")
        ...
        for underlying, options_chain in all_chains.items():
            if options_chain:
                ...
                if exp_dates:
                    ...
                    logger.debug(f"Found {len(exp_dates)} expiration dates for {underlying}")
                else:
                    logger.debug(f"No expiration dates found for {underlying}")
        
        logger.info(f"Completed options expiration collection for {len(expirations)} underlyings")
        return expirations
    
    def collect_options_strikes(
        self,
        underlyings: List[str],
        expiration_date: Union[str, datetime, date]
        ...
        """
        logger.info(f"Collecting options strikes for {len(underlyings)} underlyings for expiration {expiration_date}")
        ...
        for underlying, options_chain in all_chains.items():
            if options_chain:
                ...
                call_strikes = sorted(set(
                    ...
                    for option in options_chain 
                    if option.get('strike_price') and option.get('contract_type') == 'call'
                ))
                
                put_strikes = sorted(set(
                    option.get('strike_price') 
                    for option in options_chain 
                    if option.get('strike_price') and option.get('contract_type') == 'put'
                ))
                ...
                if call_strikes or put_strikes:
                    ...
                    logger.debug(f"Found {len(call_strikes)} call strikes and {len(put_strikes)} put strikes for {underlying}")
                else:
                    logger.debug(f"No strike prices found for {underlying}")
        
        logger.info(f"Completed options strike collection for {len(strikes)} underlyings")
        return strikes
    ...
    def collect_options_prices(
        ...
        """
        logger.info(f"Collecting options prices for {len(option_symbols)} contracts on {date_to_collect}")
    ...
    def _collect_option_prices(
        ...
        try:
            ...
            logger.debug(f"Collected {len(df)} price points for {option_symbol} on {date_str}")
            return df
            
        except Exception as e:
            logger.error(f"Error collecting prices for {option_symbol} on {date_str}: {e}")
            logger.debug(traceback.format_exc())
            return pd.DataFrame()
        ...
        for option_symbol in option_symbols:
            try:
                ...
                for i, char in enumerate(ticker_part):
                    ...
                    logger.warning(f"Could not parse underlying from option symbol: {option_symbol}")
                    continue
                ...
                if not all([strike, expiration, contract_type, underlying in underlying_prices]):
                ...
                if days_to_expiry <= 0:
                    logger.warning(f"Option {option_symbol} has expired or expires today")
                    continue
                ...
                d1 = (math.log(s / strike) + (risk_free_rate + 0.5 * iv**2) * t) / (iv * math.sqrt(t))
                d2 = d1 - iv * math.sqrt(t)
                
                if contract_type == 'call':
                    price = s * norm.cdf(d1) - strike * math.exp(-risk_free_rate * t) * norm.cdf(d2)
                    delta = norm.cdf(d1)
                    gamma = norm.pdf(d1) / (s * iv * math.sqrt(t))
                    theta = -(s * iv * norm.pdf(d1)) / (2 * math.sqrt(t)) - risk_free_rate * strike * math.exp(-risk_free_rate * t) * norm.cdf(d2)
                    vega = s * math.sqrt(t) * norm.pdf(d1) / 100  # Divided by 100 to get the change per 1% change in IV
                    rho = strike * t * math.exp(-risk_free_rate * t) * norm.cdf(d2) / 100  # Divided by 100 to get the change per 1% change in interest rate
                else:  # put
                    price = strike * math.exp(-risk_free_rate * t) * norm.cdf(-d2) - s * norm.cdf(-d1)
                    delta = norm.cdf(d1) - 1
                    gamma = norm.pdf(d1) / (s * iv * math.sqrt(t))
                    theta = -(s * iv * norm.pdf(d1)) / (2 * math.sqrt(t)) + risk_free_rate * strike * math.exp(-risk_free_rate * t) * norm.cdf(-d2)
                    vega = s * math.sqrt(t) * norm.pdf(d1) / 100
                    rho = -strike * t * math.exp(-risk_free_rate * t) * norm.cdf(-d2) / 100
