...
class QuoteCollector:
    ...
    def collect_quotes(
        ...
        """
        logger.info(f"Collecting quotes for {len(tickers)} tickers on {date_to_collect}")
    ...
    def collect_nbbo(
        ...
        """
        logger.info(f"Collecting NBBO quotes for {len(tickers)} tickers on {date_to_collect}")
    ...
    def _collect_ticker_nbbo(
        ...
        try:
            ...
            logger.error(f"Error collecting NBBO quotes for {ticker} on {date_str}: {e}")
            logger.debug(traceback.format_exc())
            return pd.DataFrame()