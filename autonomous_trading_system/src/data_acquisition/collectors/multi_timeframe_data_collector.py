...
class MultiTimeframeDataCollector:
    ...
    def __init__(
        ...
        logger.info(f"Initialized MultiTimeframeDataCollector with {self.max_threads} threads")
    ...
    def collect_multi_timeframe_data(
        ...
        if start_date is None:
            ...
            if isinstance(end_date, (datetime, date)):
                ...
                # Parse end_date string and subtract 30 days
        ...
        logger.info(f"Completed multi-timeframe data collection for {len(tickers)} tickers")
    ...
    def collect_price_data(
        ...
        logger.info(f"Completed {timeframe} price data collection for {len(results)} tickers")
        return results
    ...
    def collect_quote_data(
        ...
        """
        logger.info(f"Collecting quote data for {len(tickers)} tickers on {date_to_collect}")
        ...
        for ticker in tickers:
            future = self.executor.submit(
                self._collect_ticker_quote_data,
                ticker,
                date_str
            )
            futures[future] = ticker
    ...
    def collect_trade_data(
        ...
        """
        logger.info(f"Collecting trade data for {len(tickers)} tickers on {date_to_collect}")
        ...
        for ticker in tickers:
            future = self.executor.submit(
                self._collect_ticker_trade_data,
                ticker,
                date_str
            )
            futures[future] = ticker
    ...
    def collect_options_data(
        ...
        for ticker in tickers:
            future = self.executor.submit(
                self._collect_ticker_options_data,
                ticker,
                start_date,
                end_date,
                limit
    ...
    def _collect_ticker_price_data(
        ...
        try:
            ...
            df = self.polygon_client.get_aggregates(
                ticker, multiplier, timespan, start_date, end_date, adjusted
            )
    ...
    def _collect_ticker_quote_data(
        self,
        ticker: str,
        date_str: str
    ) -> pd.DataFrame:
    ...
    def _collect_ticker_trade_data(
        self,
        ticker: str,
        date_str: str
    ) -> pd.DataFrame:
    ...
    def _process_timeframe_data(
        ...
        for ticker, timeframe_data in data.items():
            ...
            if len(timeframes) > 1:
                processed_data[ticker] = self._generate_cross_timeframe_features(processed_data[ticker])
    ...
    def _generate_cross_timeframe_features(
        ...
        sorted_timeframes = sorted(timeframes, key=lambda tf: timeframe_minutes.get(tf, 0))
        ...
        for i, tf1 in enumerate(sorted_timeframes):
            ...
            for tf2 in sorted_timeframes[i+1:]:
                ...
                # This is a simplified example - in a real implementation, you would need more
                # sophisticated resampling logic based on the specific timeframes
                
                # For now, just add a column to the smaller timeframe DataFrame indicating
                # the relationship to the larger timeframe
                ratio_col = f"close_ratio_{tf1}_{tf2}"
                
                # Find the closest larger timeframe bar for each smaller timeframe bar
                # This is a simplified approach - in a real implementation, you would need
                # more sophisticated matching logic
                
                # For demonstration purposes, just add a placeholder column
                df1[ratio_col] = np.nan
