...
class PriceCollector:
    ...
    def collect_stock_aggs(
        ...
        logger.info(f"Completed {timespan} price data collection for {len(results)} tickers")
        return results
    ...
    def collect_daily_bars(
        ...
        """
        return self.collect_stock_aggs(tickers, 1, 'day', start_date, end_date, adjusted)
    ...
    def collect_intraday_bars(
        ...
        """
        return self.collect_stock_aggs(tickers, interval_minutes, 'minute', start_date, end_date, adjusted)
    ...
    def collect_hourly_bars(
        ...
        """
        return self.collect_stock_aggs(tickers, 1, 'hour', start_date, end_date, adjusted)
    ...
    def collect_from_alpaca(
        ...
        Returns:
            Dictionary mapping tickers to DataFrames with price data
        """
        logger.info(f"Collecting {timeframe} price data from Alpaca for {len(tickers)} tickers")
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.error(f"Error collecting {timeframe} data from Alpaca for {ticker}: {e}")
                logger.debug(traceback.format_exc())
        
        logger.info(f"Completed {timeframe} price data collection from Alpaca for {len(results)} tickers")
        return results
    ...
    def _collect_ticker_from_alpaca(
        ...
        try:
            ...
            logger.debug(f"Collected {len(df)} {timeframe} bars from Alpaca for {ticker}")
            return df
